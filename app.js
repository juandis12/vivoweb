...
async function loadPersonalizedSections() {
    ...
    if (recs.length) CATALOG_UI.renderCarousel('recommendedCarousel', recs, null, availableIds);
    
    const favorites = await loadMyList(supabase, currentProfile, availableIds);
    if (favorites.length) CATALOG_UI.renderCarousel('myListCarousel', favorites, null, availableIds);
